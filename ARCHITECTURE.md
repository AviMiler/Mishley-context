...
| `renderProjectSelect()`                                 | Populates the global `<select id="projectSelect">` (`#globalProjectBar`, above both tabs) with all projects (regular + code, code ones tagged with a plain `[קוד]` text prefix — not an emoji/icon, since native `<option>` elements can't render SVG and the folder emoji has no monochrome fallback) plus a "ללא פרויקט" option |
