...
**Dismissal model: two real conditions, not "any close" (2026-07-28).** The guide auto-opens every time the panel opens (`content.js#setPanelOpen(true)`, via `chrome.storage.local["ccb_onboardingSeen"]`, load-guarded through `loadOnboardingSeen`/`setOnboardingSeen` — the same pattern as `ctxWindow`/`docMaxChars`), not just once ever, and keeps doing so until the user satisfies one of two conditions the user chose explicitly over "just track whether it was opened": scrolling `#obBody` to within 24px of its bottom, or checking `#obDismissCheckbox` (which is visible and live-togglable every time the guide opens, not just on first install). Closing via `#obClose` or `Escape` deliberately does **not** set the seen flag. This means a user who only ever glances at the collapsed section list and closes it will keep seeing it auto-open on every future panel-open — a deliberate design, not an oversight, since the guide's own sections start collapsed and therefore often produce no scrollable overflow at all unless the user actually expands them, making the checkbox the real primary opt-out path in practice.
