...
| Piece | Behavior |
| --- | --- |
| `installSendInterceptor()` | Installed once in `chat.init()` (active-site pages only, since `initModules` runs from `mountUI`). Capture-phase listeners on **`window`** (not `document` — in the capture phase window listeners run before document listeners, so a site that delegates its send handling at document-capture level, registered before the content script, still can't read the input ahead of the prepend) for `pointerover`/`pointerdown`/`mousedown`/`click`/`keydown`. The ladder starts at **hover** (`pointerover` on the send control): the context enters the input before any press exists, so no send-trigger timing can beat it — `pointerdown` alone still lost the race on Gemini (2026-07-22, "לא מספיק להזריק בזמן"). The `[[CCB:CTX]]` marker check makes the event series idempotent (only the first prepends); a hover that never becomes a click leaves the visible prefix in the input, which is harmless — the next send carries it and nothing double-wraps. A `console.debug("[ccb] per-message context prepended via", e.type)` line reports which event won. **Inert unless the mode is `"every"`** — checked live per event, so flipping the mode needs no listener add/remove. |
