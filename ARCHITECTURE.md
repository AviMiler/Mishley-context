...
**Onboarding guide as a fourth full-pane view, and the resulting mutual-exclusion gap (2026-07-28, Phase 5 — the last feature of the batch).** `#onboardingView` (`ui-template.js`) joins `#conversationView`/`#filePreviewView`/`#depManagerView` in the same shared `.cv-shell`/`.cv-header`/`.cv-open` CSS group (`ui-styles.js`), the same fixed-position, same-z-index takeover of the panel's content area. Content is static (7 collapsed-by-default sections, icon + title + one-line-bullet items), so unlike the other three views there's nothing to render per-open — `ui-modals.js#openOnboarding`/`closeOnboarding` only handle show/hide, checkbox sync, and a one-time (`_onboardingWired` guard) delegated click listener for section collapse plus a `scroll` listener on `#obBody`. **A real bug, caught by `verify-agent`'s first Stage 3 pass, not by the main agent's own browser testing:** adding a fourth member to an N-way mutual-exclusion group requires updating all N-1 *existing* open functions to close the new one too, not just having the new one close the existing N-1 — `openOnboarding()` correctly closed the other three, but `history-view.js#openConversationView` and `code-tree.js#openPreview`/`openDepsManager` were never touched, so they didn't close onboarding back. This was reachable in practice, not theoretical: `.panel` and the four full-pane views occupy disjoint screen regions (the views sit at `left:${SIDEBAR_WIDTH}px` to `right:0`, the panel itself at `left:0`), so the panel's own buttons (a history row, a file's preview/deps-manager icons) stay fully clickable while a full-pane view is already showing — since the guide auto-opens on every panel-open (see below), clicking a history row as the very next action was a plausible sequence that would have left two views simultaneously carrying `.cv-open`. Fixed with one line per site, mirroring each function's existing sibling-close call.
