...
- [ ] **Side-panel rearchitecture (agreed, not started).** Move the UI from the injected shadow-DOM panel to a real `chrome.sidePanel`. Requires: `sidePanel` permission + `side_panel.default_path` + a background service worker (`background.js`) in `manifest.json`; a new `sidepanel.html` hosting the UI modules; and splitting the host-page DOM access (`inject.js`, the capture/observer logic in `chat-features.js`, `ctx-meter.js`'s watchers, `docHandler.injectFilesToChat`, `summarizer.js`) into a slim content script talking to the panel over `chrome.runtime` messaging. `push.js` becomes unnecessary (Chrome reflows the page natively). Pure-UI + storage modules move over unchanged. Est. 300–500 lines across ~6 files.
