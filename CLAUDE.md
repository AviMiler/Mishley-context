...
- **Context tab** — one unified view, no sub-tabs. Context-window meter at the top, then a single collapsible **`#blocksSection`** (`section-header` + `#blocksCollapseBtn`, same collapse pattern as the History tab's conversation list — toggling it hides `#blocksSectionBody` as a whole) holding **everything injectable**: the "+ new block" button, the General Memory card (`#gmCard`, always shown when expanded — global, unaffected by project selection), the active project's **instructions card** (`#projectInstructionsCard`, directly below GM, hidden when no project is active — accordion preview/edit, same pattern as GM; no separate content preview outside the accordion — `עריכה` opens the editor directly), and the **unified prompts list** (`#list`) — general blocks (no `projectId`) plus the active project's own blocks, project-owned rows carrying a `.ctx-proj-tag` pill with the project's title, always visible (not just when selected). A block created via "+" is stamped with `state.currentProjectId` if one is active, otherwise stays general (`content.js#renderUnifiedBlocksList`/`openEdit`/`saveEdit`/`syncBlocksSection`). Below `#blocksSection` (outside the collapse — its own separate toggle): for the active project, a **documents section** (`#projectDocumentsCard`, also collapsible via `#projectDocumentsToggle`/`history-view.js#syncProjectDocumentsSection`) — flat list + add-document dialog for regular projects, or the **inline file tree** (`code-tree.js#renderInline` inside `#projectDocumentsList` — tree + per-file dependency-linking, no modal) for code projects. All driven by `history-view.js#renderProjectContext`, called on every `render()`.
