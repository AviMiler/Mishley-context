...
- `window.__ccbHistoryView` — `init`, `render*`, `open*/close*`, `getProjects/getCodeProjects/getProjectById/getConversationProject`, `buildHistoryMessages/buildConversationInjectionText/buildProjectSectionText/formatTranscript`, `sync*` collapse helpers, `addProject`, `saveProjectView`, `renderProjectSelect`, `renderProjectViewDocuments`, `openProjectView/closeProjectView`, `openAddDocumentDialog`, `injectProjectDocuments`, `createCodeProjectBookmark`, `rescanCodeProject`, `loadCodeProjectAll`, `enableFilesForProject`, `setAllCodeDocsEnabled`
...
- **Context tab** — context-window meter (with file-token breakdown) at the top, then a two-button segmented sub-view control (`.ctx-subview-toggle`, wired in `content.js#syncCtxSubview`, state `state.ctxSubview`):
  ...
  - **"פרויקטים"** (`#ctxSubviewProjects`) — a project **selector** (`<select id="projectSelect">`, regular + code projects together, code ones prefixed with 📁) that stays visible at the top, and beneath it the selected project's detail view (`#projectView`): instructions accordion, the project's own text blocks (`#projectBlocksList`), documents + add-document dialog, and for code projects the **inline file tree** (via `code-tree.js#renderInline` inside `#projectDocumentsList` — tree + per-file dependency-linking, no modal). Selecting a project shows/hides the detail via `history-view.js#syncCtxProjectsLayout`; the selector itself never hides.
