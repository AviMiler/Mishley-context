...
- `window.__ccbHistoryView` — `init`, `render*` (incl. `renderProjectContext`, `renderProjectInstructionsCard`), `loadActiveProjectId/setActiveProjectId`, `open*/close*`, `getProjects/getCodeProjects/getProjectById/getConversationProject`, `buildHistoryMessages/buildConversationInjectionText/formatTranscript`, `sync*` collapse helpers, `addProject`, `renderProjectSelect`, `renderProjectViewDocuments`, `openAddDocumentDialog`, `injectProjectDocuments`, `createCodeProjectBookmark`, `rescanCodeProject`, `loadCodeProjectAll`, `enableFilesForProject`, `setAllCodeDocsEnabled`
...
- **Prompts editor** — 5 framing pairs (manual / GM / conversation wrapper / project wrapper / file-injection wrapper); the locked technical markers (`[[CCB:INJECTED]]`, `<context>`, `<memory>`, `<transcript>`, `<project>`, `<documents>`) are visible but not editable. The SUMMARY_PROMPT editor is intentionally hidden for now. The file-injection wrapper (`FRAMING_DOCS_*`) only governs `history-view.js#injectProjectDocuments()` (the footer "מסמכים" button) — the one OTHER place that builds a literal `<documents>...</documents>` block (`chat-features.js#injectSelected`, for a manually-selected project's docs) is nested *inside* an outer FRAMING_MANUAL/FRAMING_GM wrapper already and was deliberately left as-is — not a standalone top-level injection.
