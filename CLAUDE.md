...
- **Prompts editor** — 5 framing pairs (manual / GM / conversation wrapper / project wrapper / file-injection wrapper); the locked technical markers (`[[CCB:INJECTED]]`, `<context>`, `<memory>`, `<transcript>`, `<project>`, `<documents>`) are visible but not editable. The SUMMARY_PROMPT editor is intentionally hidden for now. The file-injection wrapper (`FRAMING_DOCS_*`) only governs `history-view.js#injectProjectDocuments()` (the footer "מסמכים" button) — the two OTHER places that build a literal `<documents>...</documents>` block (`chat-features.js#injectSelected` for a manually-selected project's docs, `history-view.js#buildProjectSectionText` for the conversation-continue flow) are nested *inside* an outer FRAMING_MANUAL/FRAMING_PROJ wrapper already and were deliberately left as-is — they're not standalone top-level injections.
