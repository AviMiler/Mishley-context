...
- `window.__ccbHistoryView` — `init`, `render*` (incl. `renderProjectContext`), `loadActiveProjectId/setActiveProjectId`, `open*/close*`, `getProjects/getCodeProjects/getProjectById/getConversationProject`, `buildHistoryMessages/buildConversationInjectionText/buildProjectSectionText/formatTranscript`, `sync*` collapse helpers, `addProject`, `saveProjectView`, `renderProjectSelect`, `renderProjectViewDocuments`, `openAddDocumentDialog`, `injectProjectDocuments`, `createCodeProjectBookmark`, `rescanCodeProject`, `loadCodeProjectAll`, `enableFilesForProject`, `setAllCodeDocsEnabled`
...
- **Sidebar** with FAB strip on the left, opens with click or Ctrl+Shift+L. Width is `SIDEBAR_WIDTH` in `config.js` (380px; `push.js` + full-page overlays derive from the same constant).
...
- **Context tab** — one unified view, no sub-tabs. Context-window meter at the top, then: the active project's **instructions card** (`#projectInstructionsCard`, hidden when no project is active — accordion preview/edit, same pattern as GM), the "+ new block" button, the General Memory card (`#gmCard`, always shown — global, unaffected by project selection), and a single **unified prompts list** (`#list`) combining general blocks (no `projectId`) with the active project's own blocks — project-owned rows carry a `.ctx-proj-tag` pill with the project's title, always visible (not just when selected). A block created via "+" is stamped with `state.currentProjectId` if one is active, otherwise stays general (`content.js#renderUnifiedBlocksList`/`openEdit`/`saveEdit`). Below the list: for the active project, a **documents section** — flat list + add-document dialog for regular projects, or the **inline file tree** (`code-tree.js#renderInline` inside `#projectDocumentsList` — tree + per-file dependency-linking, no modal) for code projects. All driven by `history-view.js#renderProjectContext`, called on every `render()`.
