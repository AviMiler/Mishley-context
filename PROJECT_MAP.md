...
| `msg-nav.js` | Floating prev/next widget, independent of the sidebar panel, that scrolls+highlights through the host page's own chat messages (all roles, unfiltered) using `config.js`'s existing `MSG_SELECTORS`. No storage, no shared `state` fields, and (2026-08-06 rework) no stored cursor either — position is derived fresh from the live viewport on every click; only its own module-scoped highlight-tracking state persists between calls. Exports `window.__ccbMsgNav`. |
