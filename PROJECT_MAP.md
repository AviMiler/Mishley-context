...
| `document-handler.js` | Project-document lifecycle (add/remove/toggle/estimate/extract text incl. DOCX/ODT/RTF) and the code-project folder scanner (`scanCodeProject`, `syncCodeProjectDocuments`). Exports `window.__ccbDocHandler`. |
| `dep-graph.js` | Static, no-AI import/reference graph builder for scanned code projects (JS/TS/JSX resolution + C# symbol-table heuristic). Exports `window.__ccbDepGraph`. |
