# Project Map
_Last updated: 2026-08-09 (added thinking-indicator.js — per-frame "is the chat generating a response" detector, feeding a per-tab dot on the Parallel Sessions strip)_
...
├── thinking-indicator.js          # detects "is the chat generating a response" (.waiting-indicator) — runs in every frame
├── sessions.js                    # Parallel Sessions (internal site only) — iframe-backed tab strip, per-tab thinking dot
├── content.js
...
| `msg-nav.js` | Floating prev/next widget, independent of the sidebar panel, that scrolls+highlights through the host page's own chat messages (all roles, unfiltered) using `config.js`'s existing `MSG_SELECTORS`. No storage, no shared `state` fields, and (2026-08-06) no persistent index-based cursor — position is derived from the live viewport, with a brief same-click settle-window memory (`_lastTarget`) so rapid repeated clicks don't misread a still-animating scroll. **2026-08-09:** settle detection is now a real `scrollend` listener (+ a `SETTLE_BACKSTOP_MS` safety-net timer) instead of a fixed guessed timer. Exports `window.__ccbMsgNav`. |
| `thinking-indicator.js` | New 2026-08-09. Detects, via `MutationObserver` on `MSG_SELECTORS.messageList`, whether the host chat is currently generating a response (presence of `.waiting-indicator`), mirroring `ctx-meter.js#watchConversation`'s idempotent re-arm pattern. No storage, no shared `state` fields. Runs in EVERY frame (`all_frames:true`) — pure detection, no opinion on rendering; `content.js` supplies a frame-role-aware `onChange` (self-call for the top-level/native case, `postMessage` up to `window.top` for a session iframe). `.waiting-indicator` detection itself is not yet real-browser-verified. Exports `window.__ccbThinking`. See ARCHITECTURE.md, DECISIONS.md. |
| `sessions.js` | Parallel Sessions (2026-08-06, internal site only — Gemini dropped, `X-Frame-Options: DENY`). An internal tab strip (`#sessionsView`) letting the user run several independent chat conversations on the same site in one browser tab: tab #1 ("native") is the top-level page itself, untouched; every other tab is a same-origin `<iframe>` that auto-mounts its own independent Mishley instance (`all_frames:true`, no extra wiring). Owns only the tab strip — create/close/rename/switch, storage (`ccb_sessions`, titles only). Joins the existing `.cv-shell` full-pane mutual-exclusion group; deliberately does not close on panel-close. **2026-08-09:** each tab pill also shows a live "thinking" dot, fed by `thinking-indicator.js` (direct call for native, `postMessage`+`installThinkingListener()` for session iframes). Exports `window.__ccbSessions` (gained `setNativeThinking()`). See CLAUDE.md Features overview, DECISIONS.md. |
